...
namespace CycloProxyCore
{
    public class CertMaker
    {
        ...
        public void Install()
        {
            var caName = new X509Name("CN=CycloProxyCA");
            var eeName = new X509Name("CN=CycloProxyEE");
        ...
        private AsymmetricCipherKeyPair GenerateEcKeyPair(string curveName)
        {
        ...
        private X509Certificate GenerateCertificate(X509Name issuer, X509Name subject, AsymmetricKeyParameter issuerPrivate, AsymmetricKeyParameter subjectPublic)
        {
            ISignatureFactory signatureFactory;
            if (issuerPrivate is ECPrivateKeyParameters)
            {
                signatureFactory = new Asn1SignatureFactory(X9ObjectIdentifiers.ECDsaWithSha256.ToString(), issuerPrivate);
            }
            else
            {
                signatureFactory = new Asn1SignatureFactory(PkcsObjectIdentifiers.Sha256WithRsaEncryption.ToString(), issuerPrivate);
            }
        ...
        private bool ValidateSelfSignedCert(X509Certificate cert, ICipherParameters pubKey)
        {
